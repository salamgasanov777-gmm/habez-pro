...
Что копия уехала, с сервера не видно: у его ключа нет права на список и
чтение (см. `deploy/backup.env.example` — политика ключа только `s3:PutObject`,
сроки хранения и защита от удаления настраиваются на самом бакете). Смотреть
в панели хранилища или вторым ключом «только чтение» с компьютера владельца.
...
Если что-то не так — откат сам. Если упала сама миграция, `current` не
переключается, а служба возвращается в то состояние, в каком была до
обновления (работала — запускается на старом коде); база при этом остаётся
такой, какой её оставила миграция — смотреть вывод и раздел 13.
Хранятся три последних релиза. Миграции
