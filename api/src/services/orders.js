...
export function createOrder({ tenant, user, cart, customer, deliveryCost = 0, promoCode = null, source = "web" }) {
  ...
  return tx(() => {
    ...
    const orderId = insert("orders", {
      ...
      customer_kind: customer.kind || "person",
...
export function orderView(tenantId, orderId) {
  ...
  return {
    ...
    customer: { name: order.customer_name, kind: order.customer_kind || "person", phone: order.customer_phone, email: order.customer_email, company: order.company, inn: order.inn },
